...
- `0.2.1` 已于 `2026-04-07` 发布到 VS Code Marketplace
- Marketplace 页面和 `vsce show` 结果在发布后几分钟内可能仍会有缓存延迟
...
2. 查看依赖的声明版本、最新版本和状态
3. 需要时用顶部的 package scope、筛选和搜索按钮快速缩小范围
4. 点击过时依赖，确认后执行升级
5. 也可以通过包项悬浮图标或右键菜单执行升级操作
6. 需要时可在视图标题栏执行当前可见依赖的批量保守升级
7. 对存在大版本更新的包，可以使用悬浮图标或右键菜单显式升级到最新 major
...
- [0.2.0 需求清单](./docs/release-0.2.0-plan.md)
